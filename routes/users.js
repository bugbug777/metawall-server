...
router.post('/', asyncErrorHandler(async (req, res, next) => {
  ...
  if (!name || !email || !password) appError(400, '欄位資訊不能為空！', next);

  const hasEmail = await User.findOne({email: email}).exec();
  if (hasEmail) appError(400, '該電子信箱已被使用者註冊！', next);
...
router.patch('/:id', asyncErrorHandler(async (req, res, next) => {
  ...
  if (!user) appError(400, '找不到該名使用者資訊！', next);

  if (!name || !gender) appError(400, '欄位資訊不能為空！', next);
